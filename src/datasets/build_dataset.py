...
class DatasetBuilder:
    ...
    def __init__(self, base_dir_path: Path = DIR_PATH):
        self.base_dir_path = base_dir_path
        self.output_path = base_dir_path / 'dataset.csv'
        self.queue = Queue()
        self.lock = Lock()
        self.data: list = []
    ...
    def _load_data(self):
        ...
        with ThreadPoolExecutor(max_workers=cpu_count() // 2) as executor:
            ...
            for index, path in enumerate(tqdm(paths, desc='Loading data', total=len(paths))):
                if path and path.stat().st_size > 0:
                    futures.append(executor.submit(
                        self._read_json, path, index, len(paths)))

            for future in tqdm(as_completed(futures), total=len(futures)):
                pass


    def build_dataset(self, dataset_name: str, output_path: Path = DIR_PATH / 'dataset.csv'):
        ...
        df_pd = self.data.drop_duplicates(
            subset=['document_url'])
        ...
        df_pd['text'] = df_pd['text'].apply(
            lambda x: regex.sub('\n\n\n', md(x, heading_style='ATX', strip=['img', 'a'])).strip())
...
builder.build_dataset(dataset_name,
                      output_path=dir_path / 'dataset.csv')
